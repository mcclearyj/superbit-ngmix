...
class Catalogs():
   ...
   def make_master_fitvd(self,hdu = 1,make_clean=True, clobber=True):
      ...
      """
      master_tab=Table.read(self.chipfiles[0],hdu=1) 
      for c in self.chipfiles[1:]:
         ctab=Table.read(c,hdu=1)
   ...
   def _convert_to_fiat(self, annular_cat):
      if (annular_cat.split('.')[1] !='fiat'):
         outfile_arg=annular_cat.split('.')[0]+'.fiat'
   ...
   def _make_annular_cmd(self,annular_cat,xcenter,ycenter,startrad,endrad,nbins):
      fiatcat = self._convert_to_fiat(annular_cat)
      col_args = '-c "X_IMAGE Y_IMAGE g1 g2"'
      start_arg = ' '.join(['-s',str(startrad)])
      end_arg = ' '.join(['-e',str(endrad)])
      nbins_arg = ' '.join(['-n',str(nbins)])
      catalog_arg = ' '.join([fiatcat,str(xcenter),str(ycenter)])
      outfile_arg = fiatcat.replace('.fiat','.annular')
      cmd = ' '.join(['annular',col_args,start_arg,end_arg,nbins_arg,catalog_arg,'>',outfile_arg])
   ...
   def run_annular(self, annular_cat=None, xcenter=None, ycenter=None, startrad=500, endrad=14000, nbins=35):
      ...
      cmd = self._make_annular_cmd(annular_cat,xcenter,ycenter,startrad,endrad,nbins)
      os.system(cmd)
   ...
   def run(self,coadd_catalog=None, hdu=2, filter_g=True, make_clean=True, clobber=True):
      ...
      self.make_annular_cat(hdu_ext=hdu,filter_g=filter_g)
